...
const PLANS = [
  {
    ...
    period: "forever",
    color: "#888",
    features: ["1 active project", "50 photo storage", "Email reports", "Basic tagging", "1 user"],
    limit: 50,
    cta: "Current Plan",
    ctaStyle: { background: "#F7F6F3", color: "#888", border: "1.5px solid #E5E2DC" },
  },
  {
    id: "pro",
    name: "Pro",
    ...
    color: "#E8570A",
    features: ["Unlimited projects", "Unlimited photos", "PDF & email reports", "All tags & filters", "1 user", "Priority support"],
    highlight: true,
    cta: "Start Free Trial",
    ctaStyle: { background: "#E8570A", color: "#fff" },
  },
  {
    id: "team",
    name: "Team",
    ...
    price: "$299",
    period: "/year",
    monthlyEquiv: "Just $25/mo",
    color: "#185FA5",
    features: ["Unlimited projects", "Unlimited photos", "PDF & email reports", "Up to 10 users", "Team photo sync", "Admin dashboard", "Priority support"],
    cta: "Get Team Plan",
    ctaStyle: { background: "#185FA5", color: "#fff" },
  },
  {
    id: "company",
    name: "Company",
    ...
    price: "$499",
    period: "/year",
    monthlyEquiv: "Just $41/mo",
    color: "#0F6E56",
    features: ["Unlimited projects", "Unlimited photos", "PDF & email reports", "Unlimited users", "Team photo sync", "Admin dashboard", "Client portal", "Dedicated support"],
    cta: "Get Company Plan",
    ctaStyle: { background: "#0F6E56", color: "#fff" },
  },
];

const MOCK_PROJECTS = [
  { id: 1, name: "Riverside Townhomes", client: "Apex Developers", phase: "Framing", photos: 38, lastUpdate: "2h ago", color: "#E8570A" },
  { id: 2, name: "Harbor View Office", client: "Stonebridge Corp", phase: "Electrical", photos: 12, lastUpdate: "Yesterday", color: "#0F6E56" },
  { id: 3, name: "Maple St. Renovation", client: "John & Sarah Kim", phase: "Drywall", photos: 8, lastUpdate: "3 days ago", color: "#185FA5" },
];
...
export default function SiteSnap() {
  ...
  const atLimit = currentPlan === "free" && totalPhotos >= FREE_PHOTO_LIMIT;
  const photoPercent = Math.min(Math.round((totalPhotos / FREE_PHOTO_LIMIT) * 100), 100);
  ...
  const handleUpload = () => {
    if (atLimit) { showToast("Photo limit reached — upgrade to Pro!", "warning"); setShowUpload(false); return; }
    const added = {
    ...
    <div style={{ minHeight: "100vh", background: "#F7F6F3", fontFamily: "'DM Sans', sans-serif", color: "#1A1A18" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .project-card::before { content: ''; position: absolute; left: 0; top: 0; bottom: 0; width: 4px; background: var(--accent); border-radius: 0; }
        ...
        .modal { background: #fff; border: 1.5px solid #EEEBE5; border-radius: 16px; padding: 28px; width: 90%; max-width: 520px; box-shadow: 0 20px 60px rgba(0,0,0,0.15); animation: slideUp 0.25s ease; }
        .plan-card { background: #fff; border: 1.5px solid #EEEBE5; border-radius: 14px; padding: 24px; flex: 1; min-width: 200px; transition: all 0.2s; position: relative; }
        .plan-card:hover { transform: translateY(-4px); box-shadow: 0 8px 32px rgba(0,0,0,0.1); }
        .plan-card.featured { border-color: #E8570A; }
        ...
        .toast-success { position: fixed; bottom: 24px; left: 50%; transform: translateX(-50%); background: #1A1A18; color: #fff; border-radius: 8px; padding: 12px 20px; font-size: 13px; z-index: 999; animation: toastIn 0.3s ease; white-space: nowrap; box-shadow: 0 8px 24px rgba(0,0,0,0.2); }
        .toast-warning { position: fixed; bottom: 24px; left: 50%; transform: translateX(-50%); background: #FFF8F5; color: #E8570A; border: 1.5px solid #E8570A; border-radius: 8px; padding: 12px 20px; font-size: 13px; z-index: 999; animation: toastIn 0.3s ease; white-space: nowrap; box-shadow: 0 8px 24px rgba(232,87,10,0.15); font-weight: 600; }
        .billing-toggle { display: flex; background: #F7F6F3; border-radius: 8px; padding: 3px; border: 1.5px solid #EEEBE5; }
        .billing-opt { padding: 7px 16px; border-radius: 6px; font-size: 12px; font-weight: 600; cursor: pointer; border: none; background: transparent; color: #AAA; transition: all 0.15s; font-family: 'DM Sans', sans-serif; }
        .billing-opt.active { background: #fff; color: #1A1A18; box-shadow: 0 1px 4px rgba(0,0,0,0.1); }
      ...
      <header style={{ background: "#fff", borderBottom: "1.5px solid #EEEBE5", padding: "0 28px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 60, position: "sticky", top: 0, zIndex: 50, boxShadow: "0 1px 8px rgba(0,0,0,0.05)" }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          {activeProject ? <>
            ...
            <button className="ss-btn ss-primary" onClick={() => atLimit ? setTab("pricing") : setShowUpload(true)}>
              <PlusIcon /> {atLimit ? "Upgrade to Add Photos" : "Add Photos"}
            </button>
          </> : <>
            <div style={{ fontSize: 12, color: "#AAA", fontFamily: "'DM Mono',monospace", background: "#F7F6F3", padding: "4px 10px", borderRadius: 6, border: "1px solid #EEEBE5" }}>
              FREE · {totalPhotos}/{FREE_PHOTO_LIMIT} photos
            </div>
            <button className="ss-btn ss-primary" style={{ fontSize: 12, padding: "8px 14px" }} onClick={() => { setActiveProject(null); setTab("pricing"); }}><ZapIcon /> Upgrade</button>
          </>}
        </div>
      </header>
      ...
      {!activeProject && (
        <div style={{ background: "#fff", borderBottom: "1.5px solid #EEEBE5", padding: "0 28px", display: "flex" }}>
          {["projects", "pricing"].map(t => (
            <button key={t} className={`nav-tab ${tab === t ? "active" : ""}`} onClick={() => setTab(t)}>
              {t === "projects" ? "📋 Projects" : "💳 Pricing"}
            </button>
          ))}
        </div>
      )}
      ...
      <main style={{ maxWidth: 980, margin: "0 auto", padding: "28px 20px" }}>
        ...
        {!activeProject && tab === "projects" && (
          <div className="fade-in">
            ...
            {/* Free limit bar */}
            <div style={{ background: "#fff", border: `1.5px solid ${atLimit ? "#FFAAAA" : "#EEEBE5"}`, borderRadius: 10, padding: "14px 18px", marginBottom: 20, display: "flex", alignItems: "center", gap: 16 }}>
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 6 }}>
                  <span style={{ fontWeight: 600, color: atLimit ? "#CC0000" : "#1A1A18" }}>
                    {atLimit ? "⚠ Photo limit reached" : "Free photo storage"}
                  </span>
                  <span style={{ color: "#AAA", fontFamily: "'DM Mono',monospace" }}>{totalPhotos} / {FREE_PHOTO_LIMIT}</span>
                </div>
                <div style={{ height: 6, background: "#F0EDE8", borderRadius: 3, overflow: "hidden" }}>
                  <div style={{ height: "100%", width: `${photoPercent}%`, background: atLimit ? "#CC0000" : "#E8570A", borderRadius: 3, transition: "width 0.4s ease" }} />
                </div>
              </div>
              {atLimit && (
                <button className="ss-btn ss-primary" style={{ fontSize: 12, padding: "8px 14px", whiteSpace: "nowrap" }} onClick={() => setTab("pricing")}>
                  <ZapIcon /> Upgrade Now
                </button>
              )}
            </div>
            ...
            <div style={{ display: "flex", gap: 12, marginBottom: 24 }}>
              {[{ label: "Total Photos", val: totalPhotos, color: "#E8570A" }, { label: "Flagged Issues", val: photos.filter(p=>p.flag).length, color: "#CC0000" }, { label: "Active Projects", val: MOCK_PROJECTS.length, color: "#0F6E56" }].map(s => (
        ...
        {!activeProject && tab === "pricing" && (
          <div className="fade-in">
            <div style={{ textAlign: "center", marginBottom: 32 }}>
              <h1 style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 36, letterSpacing: 2, lineHeight: 1, marginBottom: 8 }}>SIMPLE PRICING</h1>
              <p style={{ color: "#AAA", fontSize: 15, marginBottom: 20 }}>Start free. Upgrade when you're ready. Cancel anytime.</p>

              {/* Billing toggle */}
              <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 12 }}>
                <div className="billing-toggle">
                  <button className={`billing-opt ${billingCycle === "monthly" ? "active" : ""}`} onClick={() => setBillingCycle("monthly")}>Monthly</button>
                  <button className={`billing-opt ${billingCycle === "annual" ? "active" : ""}`} onClick={() => setBillingCycle("annual")}>Annual</button>
                </div>
                {billingCycle === "annual" && (
                  <span style={{ background: "#E8F5EC", color: "#1E7A3A", fontSize: 12, fontWeight: 600, padding: "4px 10px", borderRadius: 20 }}>Save up to 17%</span>
                )}
              </div>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(210px, 1fr))", gap: 14 }}>
              {PLANS.map(plan => (
                <div key={plan.id} className={`plan-card ${plan.highlight ? "featured" : ""}`} style={{ borderColor: plan.highlight ? plan.color : "#EEEBE5" }}>
                  {plan.highlight && (
                    <div style={{ position: "absolute", top: -12, left: "50%", transform: "translateX(-50%)", background: "#E8570A", color: "#fff", fontSize: 11, fontWeight: 700, padding: "4px 14px", borderRadius: 20, whiteSpace: "nowrap" }}>
                      ⚡ Most Popular
                    </div>
                  )}

                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14, marginTop: plan.highlight ? 8 : 0 }}>
                    <div style={{ width: 32, height: 32, borderRadius: 8, background: plan.color + "18", color: plan.color, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      {plan.icon}
                    </div>
                    <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 20, letterSpacing: 1, color: "#1A1A18" }}>{plan.name}</div>
                  </div>

                  <div style={{ marginBottom: 6 }}>
                    <span style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 40, letterSpacing: 1, color: plan.color }}>
                      {plan.id === "pro" && billingCycle === "annual" ? plan.annualPrice : plan.price}
                    </span>
                    <span style={{ fontSize: 13, color: "#AAA", marginLeft: 4 }}>
                      {plan.id === "pro" && billingCycle === "annual" ? plan.annualPeriod : plan.period}
                    </span>
                  </div>

                  {plan.id === "pro" && billingCycle === "monthly" && (
                    <div style={{ fontSize: 11, color: "#1E7A3A", fontWeight: 600, marginBottom: 8, background: "#E8F5EC", padding: "3px 8px", borderRadius: 20, display: "inline-block" }}>
                      Switch to annual — Save 17% →
                    </div>
                  )}
                  {plan.id === "pro" && billingCycle === "annual" && (
                    <div style={{ fontSize: 11, color: "#1E7A3A", fontWeight: 600, marginBottom: 8 }}>
                      You save $58/year vs monthly
                    </div>
                  )}
                  {plan.monthlyEquiv && (
                    <div style={{ fontSize: 11, color: "#1E7A3A", fontWeight: 600, marginBottom: 8 }}>{plan.monthlyEquiv}</div>
                  )}
                  ...
                  <button className="ss-btn" style={{ ...plan.ctaStyle, width: "100%", justifyContent: "center", padding: "11px 20px", borderRadius: 8 }}>
                    {plan.cta}
                  </button>

                  {plan.id === "free" && (
                    <div style={{ textAlign: "center", fontSize: 11, color: "#BBB", marginTop: 8 }}>No credit card required</div>
                  )}
                </div>
              ))}
            </div>
            ...
            {/* Compare table hint */}
            <div style={{ marginTop: 24, background: "#fff", border: "1.5px solid #EEEBE5", borderRadius: 10, padding: "16px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 2 }}>Need help choosing?</div>
                <div style={{ fontSize: 12, color: "#AAA" }}>Team plan is best for crews of 2–10 · Company plan for larger operations</div>
              </div>
              <div style={{ fontSize: 12, color: "#AAA", fontFamily: "'DM Mono',monospace", textAlign: "right" }}>
                <div>All plans include</div>
                <div style={{ color: "#1A1A18", fontWeight: 500 }}>14-day free trial</div>
              </div>
            </div>
          </div>
        )}
        ...
        {activeProject && tab === "photos" && (
          <div className="fade-in">
            ...
            {atLimit && (
              <div style={{ background: "#FFF8F5", border: "1.5px solid #E8570A44", borderRadius: 10, padding: "12px 16px", marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontSize: 13, color: "#E8570A", fontWeight: 600 }}>⚠ You've reached the 50 photo free limit</span>
                <button className="ss-btn ss-primary" style={{ fontSize: 12, padding: "7px 14px" }} onClick={() => { setActiveProject(null); setTab("pricing"); }}><ZapIcon /> Upgrade</button>
              </div>
            )}
            ...
            <div style={{ background:"#fff", border:"1.5px solid #EEEBE5", borderRadius:12, overflow:"hidden", boxShadow:"0 4px 20px rgba(0,0,0,0.06)" }}>
              ...
              <div style={{ padding:24 }}>
                <div style={{ display:"flex", gap:12, marginBottom:20 }}>
                  {[{l:"Report Date",v:"March 26, 2026"},{l:"Current Phase",v:activeProject.phase},{l:"Photos",v:projectPhotos.length}].map(s => (
          ...
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            {atLimit ? (
              <div style={{ textAlign:"center", padding:"20px 0" }}>
                ...
                <div style={{ fontWeight:600, fontSize:16, marginBottom:8 }}>You've hit the 50 photo limit</div>
                <div style={{ color:"#AAA", fontSize:13, marginBottom:20 }}>Upgrade to Pro for unlimited photos and projects.</div>
                <button className="ss-btn ss-primary" style={{ justifyContent:"center" }} onClick={() => { setShowUpload(false); setTab("pricing"); }}><ZapIcon /> See Upgrade Options</button>
              </div>
            ) : (
              <>
                <div style={{ border:"2px dashed #E5E2DC", borderRadius:10, padding:"28px 20px", textAlign:"center", marginBottom:18, cursor:"pointer", background:"#F7F6F3", transition:"all 0.2s" }}
                  ...
                  <div style={{ fontSize:11, color:"#CCC", marginTop:4 }}>{FREE_PHOTO_LIMIT - totalPhotos} photos remaining on free plan</div>
                </div>
      ...
      {toast && <div className={toast.type === "warning" ? "toast-warning" : "toast-success"}>{toast.msg}</div>}
    </div>
  );
}
